...
public class ISpNotifySource extends IDispatch {
	...
	int SAPINotifyCallback(int wParam, int lParam) {
		...
		try {
			while( OLE.S_OK == GetEvent(se.getAddress()) ) {
	    		...
	    		System.out.println(eventName+": "+se.getLParamType()+"/"+se.getStreamNum()+"/"+se.getAudioStreamOffset()+"/"+se.getLParam()+"/"+se.getWParam()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
			}
		}
		finally {
			se.dispose();
	...
	public int SetInterest(int eventInterest, int queuedInterest) {
		return COMUtil.VtblCall(10,address,0x40000000|eventInterest,2,0x40000000|queuedInterest,2);
	}
